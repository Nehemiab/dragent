...
# 2. 分阶段提示词列表
ANALYST_PROMPTS = [
    "你是台风灾害预警分析专家，请你先使用工具typhoon_api查询台风数据。",
    "你的助手flood是多模态大模型，可以回答图中水体位置以及水体周边的情况。接下来,请你向flood询问相应细节。请按照以下格式输出对flood的询问：\n"
    "```query to flood：'向flood询问的问题'```\n",
    "你的助手building是多模态大模型，可以回答图中房屋分布、建筑密度、脆弱性、损毁程度等情况，接下来,请你向building询问相应细节。请按照以下格式输出对building的询问：\n"
    "```query to building：'向building询问的问题'```\n",
    "你的助手road是多模态大模型，可以回答图中道路分布、通行能力、易中断路段等情况，接下来，请你向road询问相应细节。请按照以下格式输出对road的询问：\n"
    "```query to road：'向road询问的问题'```\n",
    "综合台风数据、flood/building/road 的全部信息，给出完整的风险评估、预防方案、疏散建议、所需物资，请按照以下格式输出对flood的询问和你的分析：\n"
    "```analyses：你的分析```\n"
